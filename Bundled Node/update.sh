...
if test ! -s bcprov-jdk15on-152.jar
then
	echo Downloading bcprov-jdk15on-152.jar
	if ! java $JOPTS -cp sha1test.jar Sha1Test bcprov-jdk15on-152.jar . $CAFILE
	then
		echo Could not download bcprov-jdk15on-152.jar needed for new jar
		exit
	fi
fi
...
if ! grep bcprov-jdk15on-152.jar wrapper.conf > /dev/null
then
	if grep bcprov-jdk15on wrapper.conf > /dev/null; then
		...
		echo "wrapper.java.classpath.3=bcprov-jdk15on-152.jar" >> wrapper.conf
	fi
else
	echo wrapper.conf contains up to date bouncycastle jar v1.52
	if grep bcprov-jdk15on-147.jar wrapper.conf > /dev/null; then
		echo wrapper.conf contains both bouncycastle 147 and 152, deleting 147
		cat wrapper.conf | sed "/bcprov-jdk15on-147/d" > wrapper.conf.new
		mv wrapper.conf.new wrapper.conf
	fi
	if grep bcprov-jdk15on-149.jar wrapper.conf > /dev/null; then
		echo wrapper.conf contains both bouncycastle 149 and 152, deleting 149
		cat wrapper.conf | sed "/bcprov-jdk15on-149/d" > wrapper.conf.new
		mv wrapper.conf.new wrapper.conf
	fi
	if grep bcprov-jdk15on-151.jar wrapper.conf > /dev/null; then
		echo wrapper.conf contains both bouncycastle 151 and 152, deleting 151
